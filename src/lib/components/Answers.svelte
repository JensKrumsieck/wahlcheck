<script lang="ts">
  ...
  function sortedPartiesFor(question: Question) {
    return [...parties].sort((a, b) => {
      ...
      const scoreA =
        answerA && score ? scoreMatch(score[question.id], answerA.wertung) : -1;
      const scoreB =
        answerB && score ? scoreMatch(score[question.id], answerB.wertung) : -1;
...
<h2
  id="antworten"
  class="mt-12 mb-4 scroll-mt-20 font-display text-2xl font-semibold text-ink"
>
  Antworten im Detail
</h2>
<small class="text-ink-muted"
  >ℹ️ <b>KI kann Fehler machen!</b> Überprüfe die Antworten anhand der
  <a class="underline hover:text-ink-faint" href="/list">Wahlprogramme.</a
  ></small
>
    ...
    >
      ...
      >
        ...
        <div class="flex shrink-0 items-center gap-3 pl-6 sm:pl-0">
          {#if score}<span class="text-sm text-ink-muted"
              >Du: {answerLabel(score[question.id])}</span
            >{/if}
...
<h3 class="mt-12 mb-4 font-display text-xl font-semibold text-ink">
  Antworten im Überblick
</h3>
<div class="mt-5 overflow-x-auto rounded-xl border border-border bg-surface">
  <table class="w-full border-collapse text-sm">
    <thead>
      <tr>
        <th
          class="sticky left-0 z-1 min-w-10 bg-surface py-2 pr-1 pl-3 text-left font-medium text-ink-muted"
          >#</th
        >
        {#if score}<th
            class="min-w-16 py-2 pr-1 pl-2 text-center font-semibold text-ink"
            >Du</th
          >{/if}
        {#each parties as party}
          <th class="min-w-16 py-2 pr-1 pl-2 text-center font-medium text-ink">
            <span
              class="inline-block h-2.5 w-2.5 rounded-full align-middle"
              style="background-color: {colorFor(party.name)}"
            ></span>
            <span class="ml-1 align-middle whitespace-nowrap">{party.name}</span
            >
          </th>
        {/each}
      </tr>
    ...
    <tbody>
      {#each questions as question, i}
        <tr class="border-t border-border">
          <td
            class="sticky left-0 z-1 bg-surface py-2 pr-1 pl-3 text-ink-faint"
            title={question.these}
            >{i + 1}
            <span class="text-xs text-ink-faint/60">({question.id})</span></td
          >
          {#if score}
            <td
              class="py-2 pr-1 pl-2 text-center"
              title={answerLabel(score[question.id])}
              >{answerIcon(score[question.id])}</td
            >
          {/if}
          {#each parties as party}
            {@const answer = answerFor(party, question)}
            <td
              class="py-2 pr-1 pl-2 text-center"
              title={answer
                ? `${party.name}: ${answerLabel(answer.wertung)}`
                : `${party.name}: keine Antwort`}
            >
              {answer ? answerIcon(answer.wertung) : "–"}
            </td>
          {/each}
        </tr>
      {/each}
